function plot_4bars_withinError_3subplots(data, t_values, reference, ylabel_string, ylimit)
...
for effect=1:3
    ...
    for i=1:size(data{effect},2)
        ...
        plot([i+shift,i+shift], [mean(data{effect}(:,i))-std(data{effect}(:,i))/sqrt(number_subjects), ...
            mean(data{effect}(:,i))+std(data{effect}(:,i))/sqrt(number_subjects)], 'k', 'LineWidth',2);
        plot([i+shift-.05,i+shift+.05], [mean(data{effect}(:,i))-std(data{effect}(:,i))/sqrt(number_subjects), ...
            mean(data{effect}(:,i))-std(data{effect}(:,i))/sqrt(number_subjects)], 'k', 'LineWidth',2);
        plot([i+shift-.05,i+shift+.05], [mean(data{effect}(:,i))+std(data{effect}(:,i))/sqrt(number_subjects), ...
            mean(data{effect}(:,i))+std(data{effect}(:,i))/sqrt(number_subjects)], 'k', 'LineWidth',2);
    end
